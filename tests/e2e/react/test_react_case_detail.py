...
def _navigate_to_first_case(page):
    """Helper to navigate to the first case's detail page via the cases list."""
    react_navigate(page, "/app/cases")
    wait_for_loading_gone(page)
    first_row = page.locator("tbody tr").first
    # Use expect_response to wait for the case detail API call
    # (SPA navigation is instant — networkidle fires before XHR starts)
    with page.expect_response(lambda r: "/api/v1/cases/" in r.url and r.request.method == "GET", timeout=15000):
        first_row.click()
    # Now the API has responded — wait for React to re-render
...
class TestHeroSection:
    ...
    def test_austlii_link(self, react_page):
        _navigate_to_first_case(react_page)
        link = react_page.get_by_text("View on AustLII")
...
class TestMetadataGrid:
    ...
    def test_metadata_heading(self, react_page):
        _navigate_to_first_case(react_page)
        assert react_page.locator("h2").get_by_text("Metadata").is_visible()
...
class TestActionButtons:
    ...
    def test_back_button_visible(self, react_page):
        _navigate_to_first_case(react_page)
        back = react_page.locator("main").get_by_text("Back")
