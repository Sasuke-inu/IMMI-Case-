...
import type { LineageData, CourtLineage, CourtMetadata } from "@/lib/lineage-data";
...
/** Key year tick marks on the axis */
const AXIS_YEARS = [2000, 2005, 2010, 2015, 2020, 2025];

function yearToPercent(year: number): number {
  return Math.min(100, Math.max(0, ((year - TIMELINE_START) / TIMELINE_SPAN) * 100));
}
...
function YearAxis() {
  return (
    <div className="flex items-center gap-3 mb-0.5">
      {/* Left spacer: matches court label column */}
      ...
      {/* Right spacer: matches year range column */}
  ...
  return (
    <div className="flex items-center gap-3 py-0.5">
      ...
      <div className="relative flex-1 h-6">
        ...
        <div
          className={cn(
            "absolute inset-y-0 flex items-center overflow-hidden rounded-sm",
          ...
          title={`${court.code}: ${court.years[0]}–${isOngoing ? "present" : court.years[1]} · ${totalCases.toLocaleString()} cases`}
        >
          {widthPct > 8 && (
        ...
        {/* Ongoing pulse at trailing edge */}
        {isOngoing && (
          <div
      ...
      <div className="w-[72px] shrink-0 text-[10px] text-muted-text tabular-nums">
        {court.years[0]}–{isOngoing ? "now" : court.years[1]}
      </div>
    </div>
  );
}
        ...
        >
          ...
          <div className="min-w-0">
            <p className="text-xs font-semibold text-foreground">
              <span className="font-mono text-accent">{t.from}</span>
              <span className="mx-1 text-muted-text">→</span>
              <span className="font-mono text-accent">{t.to}</span>
            </p>
...
function LineageGroup({ lineage }: LineageGroupProps) {
  const Icon = lineage.id === "lower-court" ? Scale : Building2;
  const totalCases = useMemo(
    () =>
      lineage.courts.reduce(
        (sum, c) => sum + Object.values(c.case_count_by_year).reduce((a, b) => a + b, 0),
        0,
      ),
    [lineage],
  );
  ...
  return (
    <div>
      ...
      <div className="mb-1.5 flex items-center gap-2">
        ...
        <span className="text-xs font-semibold text-foreground">{lineage.name}</span>
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4">
      <h3 className="mb-4 font-heading text-sm font-semibold text-foreground">
        {t("lineage.succession_timeline", "Court Succession Timeline")}
      </h3>

      {/* Shared year axis – aligns with all GanttRows below */}
      <YearAxis />

      {/* Lineage groups */}
      <div className="mt-1 space-y-5">
        {data.lineages.map((lineage) => (
          <LineageGroup key={lineage.id} lineage={lineage} />
        ))}
      </div>
    </div>
  );
